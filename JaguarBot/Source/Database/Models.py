...
def generateDefaults(engine):
    ...
    with Session(engine) as session:
        nso = AppVersion(name="NSOVersion", version=NSO_VERSION)
        s3  = AppVersion(name="S3Version", version=S3_VERSION)
        gql = AppVersion(name="GraphQLHashes", version=GQL_HASHES)

        session.add_all([nso, s3, gql])
        session.commit()
